...
// Instanciamos o objeto da nossa Classe
  ...
  const atualizarResposta = (itemId: string, valor: boolean, descricao: string) => {
    // Se o usuário clicar no que já está selecionado, removemos a resposta
    if (respostas[itemId]?.segue === valor) {
      const novasRespostas = { ...respostas };
      delete novasRespostas[itemId];
      setRespostas(novasRespostas);
    } else {
      // Caso contrário, define a nova resposta normalmente
      ...
      return (
        <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
          <div className="p-8 bg-white shadow-xl rounded-lg text-center border-t-4 border-green-600">
            ...
            <p className="text-gray-600 mb-6">Acesse com seu e-mail INSTITUCIONAL para continuar</p>
            <button
              onClick={handleLogin}
 ...
 return (
    <>
      {/* O Banner controla o estado 'podeInteragir' */}
      <BannerTermos aoAceitar={() => setPodeInteragir(true)} tempo={10} />

      {/* Esta DIV agora envolve TODO o conteúdo abaixo */}
      <div className={`transition-all duration-700 min-h-screen bg-gray-50 ${
        ...
        <div className="flex flex-col items-center p-6 pt-32">
          <div className="w-full max-w-md bg-white p-8 rounded-lg shadow-md border border-gray-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-10">
              ...
              {itens.map((item) => (
                <div key={item.id} className="p-4 border-b-2 border-gray-100 space-y-4">
                  ...
                  <div className="flex gap-4">
                    {/* Opção SIM */}
                    <div 
                      className={`flex flex-1 items-center justify-between p-3 border rounded-md cursor-pointer transition ${respostas[item.id]?.segue === true ? 'bg-green-50 border-green-500' : 'bg-white'}`}
                    ...
                    {/* Opção NÃO */}
                    <div 
                      className={`flex flex-1 items-center justify-between p-3 border rounded-md cursor-pointer transition ${respostas[item.id]?.segue === false ? 'bg-red-50 border-red-500' : 'bg-white'}`}
                  ...
                  {/* Justificativa OBRIGATÓRIA apenas se clicar em NÃO */}
              ...
              <button
                type="submit"
                // Botão liberado mesmo sem responder todos os itens
                className="w-full bg-green-700 text-white py-4 rounded-md font-bold hover:bg-green-800 transition shadow-lg disabled:bg-gray-300"
              >
                SALVAR AVALIAÇÕES ({Object.keys(respostas).length} respondidos)
              </button>
            </form>
          </div>
        </div>
